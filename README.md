...
• For now I have just set them to a 5 minute interval between tasks in order starting at 1:00 AM
...
1) Customers - will be loaded first, as that is the top of the data heirarchy. Policies has a fk to customers
...
2)  Merge from ingest into collect database
    This is a secondary ingestions with version history and timestamps
    Only inserts new row if there is new data or if data has changed
    NOTE: I would do #1 and #2 in one step, but I have not yet figured out how in snowflake
...
4)  Overwrite from collect transform into present database. This is where we answer the questions
    TO DO: Which policy types are generating the highest loss ratios? 
    TO DO: How do claims and payments trend across different policy segments? 
    TO DO: Which customers or policy groups may represent elevated financial risk? 
    TO DO: How can leadership monitor policy profitability and claims exposure over time? 
...
NOTE: since there was one I'm concerned there may be more in the future that aren't as obviously illigitimate
...
POTENTIAL ALTERNATIVE: switch the value to negative for Coverage Decrease representing an decrease in income
POTENTIAL ALTERNATIVE: switch the value to negative for Cancellation representing an decrease in income
...
PREVIOUS SOLUTION: remove where expiration_date less than effective_date policies_transform process
...
NOTE: in a real pipeline, I would have disucssions about source and destination with stakeholders to make sure the data is in the best format, whether that be eliminating the bad data, or pointing it at a 'dummy'/'unknown' foreign key.
